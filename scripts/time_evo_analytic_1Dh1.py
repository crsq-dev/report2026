...
from crsq_xp.classic import params, reference1d
...
def run_simulation(par: params.Params, basedir: str):
    ...
    ref = reference1d.Analytic1D(par, basedir, psifunc=psifunc)
    ref.run_calculation()
    ref.generate_animation_frames()


def run():
    parser = argparse.ArgumentParser(
        prog="time_evo_analytic_h1",
        description="Analytical solution for time evolution of a hydrogen atom in 1D",
    )
    parser.add_argument("--bits", type=int, default=6, help="Number of bits for the coordinate (default=6)")
    ...
    parser.add_argument("--length", type=float, default=16.0, help="Length of the 1D grid (default=16.0)")
    ...
    parser.add_argument("--qn", type=int, default=1, help="Quntum number for the wave function (default=1)")
    parser.add_argument("--delta-t", type=float, default=0.001, help="Time step (default=0.001)")
    parser.add_argument("--window-radius", type=int, help="P-space window radius (default=2^(bits-1))")
    parser.add_argument("--total-time", type=float, default=30.0, help="Total time (default=30.0)")
    parser.add_argument("--interval-time", type=float, default=0.1, help="Interval time (default=0.1)")
    parser.add_argument(
        "--use-saved-data",
        help="skip calculation and use logged data",
        action="store_true",
    )
    parser.add_argument("--dpi", type = int, default=None, help ="DPI for animation frames (default=None)")
    ...
    configpath = f"1D_{args.bits}b_{slabel}/N{args.qn}_L{args.length}_X{args.x0}_WM{window_radius}/dt{args.delta_t}/{num_nucl}n.{num_elec}e"
    outdir = "onedrive.lnk/analytic1D/" + configpath
    os.makedirs(outdir, exist_ok=True)
    logfilename = outdir + "/analytic_h1.log"
