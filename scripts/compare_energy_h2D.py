...
def compare_energy(
    ...
    analytic_filename = f"{basedir}/analytic2D/{bits}b_{signed_label}_L{length}_n{qnum_n}_m{qnum_m}/dt{delta_t}_T{total_time}/energy_trace.csv"
    if os.path.exists(analytic_filename):
        ...
        hp_df[key] = analytic_df["Hp_4"]
        htot_df[key] = analytic_df["Hk+Hp_4"]
        ...
        for k, kk in [ ("Ek","Hk"), ("Ep", "Hp_4"), ("Etot", "Hk+Hp_4")]:
            analytic_df[f"{k} analytic_discretized"] = analytic_df[kk]
    else:
        raise ValueError(f"analytic file not found: {analytic_filename}")

    pole="r0lim_0.250"
    classic_filename = f"{basedir}/classic2D/n{qnum_n}_m{qnum_m}_TO{trotter_order}_{pole}/{bits}b_L{length}_T{total_time}_iT{interval_time}_dt{delta_t}/energy_trace.csv"
    ...
    stqr_filename = f"{basedir}/circuits2D/GPU_cuStateVec_{precision}/n{qnum_n}_m{qnum_m}_TO{trotter_order}_{pole}/{bits}b_L{length}_T{total_time}_iT{interval_time}_dt{delta_t}/energy_trace.csv"
    ...
    compare_csv_dirname = f"{basedir}/compare2D/{bits}b{slabel}_L{length}_TO{trotter_order}/n{qnum_n}_m{qnum_m}/dt{delta_t}/{num_nucl}n.{num_elec}e"
