...
def draw_one_frame(par, out_dir: str, t: float):
    Q0 = par.x0
    logger.info("Q0=%f", Q0)
    odd_flag = True
    use_palma = False
    ...
    drawer = draw_frame.DrawFrame1d(par, out_dir, psifunc, vfunc)
    drawer.draw_frame(t)
...
def run():
    ...
    parser.add_argument(
        "--fixed-point",
        help="Use fixed point arithmetic", action="store_true"
    )
    parser.add_argument(
        "--length",
        type=float,
    ...
    parser.add_argument("--time", type=float, required=True, help="Time to draw the frame")
    ...
    if args.fixed_point:
        ...
        fp_label = ""
    configpath = f"1D_{args.bits}b_{slabel}{fp_label}/N{args.qn}{parity}_L{args.length}_X{args.x0}_WM{window_radius}/dt{args.delta_t}/{num_nucl}n.{num_elec}e"
    ...
    draw_one_frame(par,outdir, args.time)
