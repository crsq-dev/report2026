...
def run():
    ...
    parser.add_argument(
        ...
        type=str,
        default="3d-3qp",
        choices=["2d", "3d", "3d-re", "3d-3", "3d-3qp"],
    )
    ...
    num_elec = int(args.interval_time / args.delta_t)
    num_nucl = int(args.total_time / args.interval_time)

    configpath = f"GPU_cuStateVec_double_{args.bits}b{slabel}/n{args.qnum_n}_m{args.qnum_m}_L{args.length}/dt{args.delta_t}/{num_nucl}n.{num_elec}e"
    outdir = "onedrive.lnk/circuits2D/" + configpath
    os.makedirs(outdir, exist_ok=True)
    logfilename = outdir + "/frame_2D.log"
